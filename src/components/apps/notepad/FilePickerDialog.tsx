...
export function FilePickerDialog({
  ...
  const entries = useMemo(() => {
    ...
    const showAll = !extensions || extensions.length === 0;
    return nodes
      .filter((n) => n.type === 'directory' || showAll || extensions.includes(fileExt(n.name)))
      .sort((a, b) => {
        if (a.type !== b.type) return a.type === 'directory' ? -1 : 1;
        return a.name.toLowerCase().localeCompare(b.name.toLowerCase());
      });
  }, [listDir, currentDir, extensions]);
  ...
  const handleConfirm = useCallback(() => {
    ...
    // Auto-append the first extension for Save if the user omitted one.
    if (mode === 'save' && extensions && extensions.length > 0 && !fileExt(name)) {
      name = `${name}.${extensions[0]}`;
    }
    onConfirm(joinPath(currentDir, name));
  }, [fileName, currentDir, getNode, mode, extensions, onConfirm]);
