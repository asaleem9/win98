...
export default function VisualBasic6({ windowId }: AppComponentProps) {
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (isEditing) return;
      if ((e.key === 'Delete' || e.key === 'Backspace') && selection !== 'form') {
        setControls((cs) => removeControl(cs, selection));
        setSelection('form');
      }
    };
  ...
  const renderControl = (control: VbControl) => {
    const isSelected = selection === control.id;
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--win98-button-face)] font-[family-name:var(--win98-font)] text-[11px] select-none" data-window-id={windowId}>
      ...
      <div className="flex items-center h-[26px] px-1 gap-[1px] border-b border-[var(--win98-button-shadow)]">
        ...
        <button className="w-[23px] h-[22px] flex items-center justify-center border border-transparent hover:win98-flat-raised text-[11px]">📋</button>
        <div className="w-px h-[18px] mx-[2px] border-l border-[var(--win98-button-shadow)] border-r border-r-[var(--win98-button-highlight)]" />
        <button className="w-[23px] h-[22px] flex items-center justify-center border border-transparent hover:win98-flat-raised text-[14px]" onClick={handleRun} title="Run (F5)" disabled={runMode}>▶</button>
        ...
        <button className="w-[23px] h-[22px] flex items-center justify-center border border-transparent hover:win98-flat-raised text-[11px]" title="Stop" onClick={handleStop} disabled={!runMode}>⏹</button>
        <div className="w-px h-[18px] mx-[2px] border-l border-[var(--win98-button-shadow)] border-r border-r-[var(--win98-button-highlight)]" />
        <button
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="bg-[var(--win98-button-face)] px-1 py-[2px]">
              <select
                ...
                onChange={(e) => setSelection(e.target.value === '__form__' ? 'form' : e.target.value)}
              >
